...
public class VentaKafkaProducer {
  ...
  public CompletionStage<Void> publishVentaCreatedEvent(Venta venta) {
    ...
    try {
      ...
      // El método send(Message) es void, así que lo envolvemos en CompletableFuture
      ventaEmitter.send(message);
      ...
      LOG.errorf(e, "Failed to publish venta.created event: ventaId=%s", venta.getId());
      return CompletableFuture.failedFuture(
          new KafkaPublishException("Failed to publish event to Kafka", e)
      );
    }
  }
  ...
  private Message<VentaEventDto> buildKafkaMessage(VentaEventDto event, String key) {
    OutgoingKafkaRecordMetadata<String> metadata = OutgoingKafkaRecordMetadata.<String>builder()
        .withKey(key)
        .addHeaders(
            new RecordHeader("event-type", event.getEventType().getBytes(StandardCharsets.UTF_8)),
            new RecordHeader("event-version",
                event.getEventVersion().getBytes(StandardCharsets.UTF_8)),
            new RecordHeader("event-id", event.getEventId().getBytes(StandardCharsets.UTF_8))
        )
        .build();
